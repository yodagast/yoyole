...
async def seed_all() -> None:
    ...
    async with async_session_factory() as db:
        ...
        # 2) 分类与商品（仅空库时写入示例数据；已有真实商品时跳过，
        #    避免重启时重建旧的示例商品覆盖独立站数据）
        has_product = await db.execute(select(Product.id).limit(1))
        if has_product.scalar_one_or_none() is None:
